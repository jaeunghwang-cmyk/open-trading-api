...
from core.signal_runner import run_runner_cycle
...
_signal_runner_task: asyncio.Task | None = None
...
async def _signal_runner_loop():
    while True:
        try:
            await run_runner_cycle()
        except Exception as exc:
            print(f"[signal-runner] {exc}")
        await asyncio.sleep(5)


@app.on_event("startup")
async def on_startup():
    global _execution_sync_task, _signal_runner_task
    if _execution_sync_task is None:
        _execution_sync_task = asyncio.create_task(_execution_sync_loop())
    if _signal_runner_task is None:
        _signal_runner_task = asyncio.create_task(_signal_runner_loop())
...
async def on_shutdown():
    ...
    if _signal_runner_task is not None:
        _signal_runner_task.cancel()
        _signal_runner_task = None
